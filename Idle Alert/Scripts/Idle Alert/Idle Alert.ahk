...
; It configures how window titles are matched when using commands like WinActivate, WinExist, WinWait, and others that target windows based on their titles
	SetTitleMatchMode, 2

; Set the script to only interact if the target window activate
	#IfWinActive, ahk_class fenwindow

; #Include is a directive, which means it's processed before the script starts running
	#Include, %A_ScriptDir%\1st Object.txt
	#Include, %A_ScriptDir%\2nd Object.txt

;*************************************************************************************************************************
; Menu Tray
;*************************************************************************************************************************
	Menu, Tray, Icon, %A_ScriptDir%\Images\Sandglass.ico
	Menu, Tray, Tip, % "Idle Alert"
;*************************************************************************************************************************
;*************************************************************************************************************************
...
; Saved Gui Position
	IniRead, xPos, %A_ScriptDir%\Settings.ini, SavedGuiPosition, xPos, ""
	IniRead, yPos, %A_ScriptDir%\Settings.ini, SavedGuiPosition, yPos, ""

; Idle Time - Seconds
	IniRead, Tick, %A_ScriptDir%\Settings.ini, Idle Time, Seconds, %A_Space%
	Transform, Tick, Deref, %Tick%
...
; Set Sound Effects
	...
	Crickets_Chirping = %A_ScriptDir%\Sound\Alert\No Sound\MP3\Cricket.mp3
	Wind_Blowing = %A_ScriptDir%\Sound\Alert\Gentle Sound\MP3\Wind Blowing.mp3
	Ocean_Waves_Sound = %A_ScriptDir%\Sound\Alert\Ocean Waves\MP3\Ocean Waves Sample.mp3
	Beep_Sound = %A_ScriptDir%\Sound\Alert\Persistent Sound\MP3\Beep.mp3
	Heart_Beat_Sound = %A_ScriptDir%\Sound\Alert\Heart Beat\MP3\Heart Beat Sample.mp3
	Music_Sound = %A_ScriptDir%\Sound\Alert\Music\MP3\Star Dust Sample.mp3
...
; Gui Initializing
	Gui, +ToolWindow +AlwaysOnTop -Caption +HwndGuiHwnd
	Gui, Color, B19CD9
; Gui Title
	Gui, Font, s13 cDefault Underline Bold, Verdana
	Gui, Add, Picture, x5 y7 w24 h24 gGuiMove, %A_ScriptDir%\Images\Sandglass.ico
	Gui, Add, Text, x33 y10 gGuiMove, Idle Alert
	Gui, Margin, 0, 0
; Gui Text Display
	Gui, Font, s16 cDefault Norm Bold, Verdana
	Gui, Add, Text, c8B0000 x5 y35 w170 Border Center vWork_Status, Stop
; Gui Buttons
	Gui, Font, s12, Verdana
	Gui, Add, Button, x+5 yp+4 w60 h20 vBtnStartStop gStartStop Default, Start
	Gui, Font, s10, Verdana
	Gui, Add, Button, xp y+45 w60 h40 vBtnScanRefresh gScanRefresh, Scan`nObject
	Gui, Add, Button, xp y+6 w60 h40 vBtnCheckObject gCheck_Object, Check`nObject
	Gui, Add, Button, xp y+45 w60 h20 vBtnAbout gAbout, About
	Gui, Add, Button, xp y+6 w60 h20 vBtnGitHub gGitHub, GitHub
	Gui, Add, Button, xp y+6 w60 h20 vBtnVideo gVideo, Video
; Set Idle Time
	Gui, Font, Norm Bold, Verdana
	Gui, Font, s10, Verdana
	Gui, Add, Text, c00468B x5 y70 vSet_Idle_Time, Idle Time:
	Gui, Font, s10, Verdana
	Gui, Add, Edit, Limit2 Number x+4 w36 h20 vTick gSubmit_Initial_Time, %Tick%
	Gui, Font, s8, Verdana
	Gui, Add, Text, cBlack x+4 vSeconds, seconds
; Flash Alert
	Gui Font, s8 Underline Bold, Verdana
	Gui, Add, Text, cBlack x5 y95 vFlash_Alert_Text, Flash Alert
	Gui, Font, Norm Bold, Verdana
	Gui, Add, CheckBox, c8B0000 x5 y+5 vFlash_Alert gSubmit_Checkbox_Flash, - Flash Alert
	Gui, Add, Radio, cFA2A55 x5 y+5 vMonitor_1 gSubmit_Radio_Monitor, - Primary Monitor
	Gui, Add, Radio, c6C5809 x5 y+5 vMonitor_2 gSubmit_Radio_Monitor, - Secondary Monitor
	Gui, Add, Radio, c2B604D x5 y+5 vMonitor_3 gSubmit_Radio_Monitor, - All Monitors
; Sound Alert
	Gui Font, s8 Underline Bold, Verdana
	Gui, Add, Text, cBlack x5 y190 vSound_Alert_Text, Sound Alert
	Gui, Font, Norm Bold, Verdana
	Gui, Add, Radio, c444444 x5 y+5 vNo_Sound gSubmit_Radio_Sound, - No Sound
	Gui, Add, Radio, c1E7E1E x5 y+5 vGentle_Sound gSubmit_Radio_Sound, - Gentle Sound
	Gui, Add, Radio, c2B65EC x5 y+5 vOcean_Waves gSubmit_Radio_Sound, - Ocean Waves
	Gui, Add, Radio, c5A5A58 x5 y+5 vPersistent_Sound gSubmit_Radio_Sound, - Persistent Sound
	Gui, Add, Radio, cC11C84 x5 y+5 vHeart_Beat gSubmit_Radio_Sound, - Heart Beat
	Gui, Add, Radio, c78620A x5 y+5 vMusic_Alert gSubmit_Radio_Sound, - Music Alert
; Gui Close Button
	Gui, Font, s10, Verdana
	Gui, Add, Picture, x219 y2 w24 h24 vClose_Window gGuiClose, %A_ScriptDir%\Images\Close Window.png
; INI Settings - Flash Alert
	IniRead, Flash_Alert, Settings.ini, Flash Alert, Flash Alert, %A_Space%
	If (Flash_Alert = 0)
	{
		GuiControl, , Flash_Alert, 0
		GuiControl, Disable, Monitor_1
		GuiControl, Disable, Monitor_2
		GuiControl, Disable, Monitor_3
	}
	If (Flash_Alert = 1)
	{
		GuiControl, , Flash_Alert, 1
		GuiControl, Enable, Monitor_1
		GuiControl, Enable, Monitor_2
		GuiControl, Enable, Monitor_3
	}
	IniRead, Monitor_1, %A_ScriptDir%\Settings.ini, Flash Alert, Primary Monitor, %A_Space%
	If (Monitor_1 = 0)
		GuiControl, , Monitor_1, 0
	If (Monitor_1 = 1)
		GuiControl, , Monitor_1, 1
	IniRead, Monitor_2, %A_ScriptDir%\Settings.ini, Flash Alert, Secondary Monitor, %A_Space%
	If (Monitor_2 = 0)
		GuiControl, , Monitor_2, 0
	If (Monitor_2 = 1)
		GuiControl, , Monitor_2, 1
	IniRead, Monitor_3, %A_ScriptDir%\Settings.ini, Flash Alert, All Monitors, %A_Space%
	If (Monitor_3 = 0)
		GuiControl, , Monitor_3, 0
	If (Monitor_3 = 1)
		GuiControl, , Monitor_3, 1
; INI Settings - Sound Alert
	IniRead, No_Sound, %A_ScriptDir%\Settings.ini, Sound Alert, No Sound, %A_Space%
	If (No_Sound = 0)
		GuiControl, , No_Sound, 0
	If (No_Sound = 1)
		GuiControl, , No_Sound, 1
	IniRead, Gentle_Sound, %A_ScriptDir%\Settings.ini, Sound Alert, Gentle Sound, %A_Space%
	If (Gentle_Sound = 0)
		GuiControl, , Gentle_Sound, 0
	If (Gentle_Sound = 1)
		GuiControl, , Gentle_Sound, 1
	IniRead, Ocean_Waves, %A_ScriptDir%\Settings.ini, Sound Alert, Ocean Waves, %A_Space%
	If (Ocean_Waves = 0)
		GuiControl, , Ocean_Waves, 0
	If (Ocean_Waves = 1)
		GuiControl, , Ocean_Waves, 1
	IniRead, Persistent_Sound, %A_ScriptDir%\Settings.ini, Sound Alert, Persistent Sound, %A_Space%
	If (Persistent_Sound = 0)
		GuiControl, , Persistent_Sound, 0
	If (Persistent_Sound = 1)
		GuiControl, , Persistent_Sound, 1
	IniRead, Heart_Beat, %A_ScriptDir%\Settings.ini, Sound Alert, Heart Beat, %A_Space%
	If (Heart_Beat = 0)
		GuiControl, , Heart_Beat, 0
	If (Heart_Beat = 1)
		GuiControl, , Heart_Beat, 1
	IniRead, Music_Alert, %A_ScriptDir%\Settings.ini, Sound Alert, Music Alert, %A_Space%
	If (Music_Alert = 0)
		GuiControl, , Music_Alert, 0
	If (Music_Alert = 1)
		GuiControl, , Music_Alert, 1
; Gui Finalize
	Draw_Outline(Gui_Name := 1, x := 0, y := 0, w := 245, h := 320, Color1 := "AA336A", Color2 := "AA336A", Thickness := 2)
	Gui, Show, x%xPos% y%yPos% w245 h320, Idle Alert
	WinActivate, ahk_class fenwindow
	Return
...
GuiMove:
	...
	IniWrite, %xPos%, %A_ScriptDir%\Settings.ini, SavedGuiPosition, xPos
	IniWrite, %yPos%, %A_ScriptDir%\Settings.ini, SavedGuiPosition, yPos
	Return
...
About:
	...
	MsgBox, 4128, Idle Alert, Idle Alert is an application for Brighter Shores, a PC game created by Andrew Gower, the creator of RuneScape. The purpose of this app is to alert you whenever you're idle during crafting or combat. I’ve included a video guide in the app for anyone who needs assistance.`n`nIdle Alert is an open-source application, and I want this project to reach as many people as possible.
	SoundPlay, %Button_Click%
	Return
...
GitHub:
	MsgBox, 4148, Idle Alert, You are about to visit an external link to Idle Alert's GitHub page. Click 'Yes' to proceed.
	IfMsgBox, Yes
		Run, https://github.com/Arclayton/Idle-Alert.git
	SoundPlay, %Button_Click%
	Return
...
Video:
	MsgBox, 4148, Idle Alert, You are about to visit an external link to Idle Alert’s YouTube video guide. Click 'Yes' to proceed.
	IfMsgBox, Yes
		Run, https://youtu.be/uSJcTbjP-s4
	SoundPlay, %Button_Click%
	Return
...
ScanRefresh:
	...
	GuiControlGet, BtnScanRefresh
	If (BtnScanRefresh = "Scan`nObject")
	{
		GuiControl, , BtnScanRefresh, Refresh
		GuiControlGet, BtnScanRefresh
		Gui, Font, s9, Verdana
		GuiControl, Font, BtnScanRefresh
		Run, "%A_ScriptDir%\Scan Object.exe"
		Run, "%A_ScriptDir%\FindText\Find Text.exe"
	}
	Else
	{
		If WinExist("ahk_exe Find Text.exe")
		{
			APP_1 = %A_ScriptDir%\FindText\Find Text.exe
			WinClose, ahk_exe %APP_1%
			WinGet, xPID, PID, %APP_1%
			Process, Close, %xPID%
		}
		If WinExist("ahk_exe Scan Object.exe")
		{
			APP_2 = %A_ScriptDir%\Scan Object.exe
			WinClose, ahk_exe %APP_2%
			WinGet, xPID, PID, %APP_2%
			Process, Close, %xPID%
		}
	; Saving Input Data into INI File
		GuiControlGet, Tick
		StringReplace, Tick, Tick, `n, ``n, All
		IniWrite, %Tick%, %A_ScriptDir%\Settings.ini, Idle Time, Seconds
		Gui, Destroy
		Sleep, 75
		Reload
	}
...
StartStop:
	SoundPlay, %Button_Press%
	GuiControlGet, BtnStartStop
	If (BtnStartStop = "Start")
	{
		SetTimer, Idle_State, 1000
		Idle_Time_Lock := 1
		GuiControl, , BtnStartStop, Stop
		GuiControl, , Work_Status, Not Working
		Gui, Font, s16 cRed Norm Bold, Verdana
		GuiControl, Font, Work_Status
		GuiControl, Hide, Set_Idle_Time
		GuiControl, Hide, BtnScanRefresh
		GuiControl, Hide, BtnCheckObject
		GuiControl, Hide, BtnAbout
		GuiControl, Hide, BtnGitHub
		GuiControl, Hide, BtnVideo
		GuiControl, Hide, Flash_Alert_Text
		GuiControl, Hide, Flash_Alert
		GuiControl, Hide, Monitor_1
		GuiControl, Hide, Monitor_2
		GuiControl, Hide, Monitor_3
		GuiControl, Hide, Sound_Alert_Text
		GuiControl, Hide, No_Sound
		GuiControl, Hide, Gentle_Sound
		GuiControl, Hide, Ocean_Waves
		GuiControl, Hide, Persistent_Sound
		GuiControl, Hide, Heart_Beat
		GuiControl, Hide, Music_Alert
		GuiControl, Hide, Tick
		GuiControl, Hide, Seconds
		Draw_Outline(Gui_Name := 1, x := 0, y := 0, w := 245, h := 67, Color1 := "AA336A", Color2 := "AA336A", Thickness := 2)
		Gui, Show, w245 h67, Idle Alert
	}
	Else
	{
		SetTimer, Idle_State, Off
		SetTimer, Working_State, Off
		SetTimer, Idle_Countdown, Off
		GuiControl, , BtnStartStop, Start
		GuiControl, Show, Flash_Alert_Text
		GuiControl, Show, Flash_Alert
		GuiControl, Show, Monitor_1
		GuiControl, Show, Monitor_2
		GuiControl, Show, Monitor_3
		GuiControl, Show, Sound_Alert_Text
		GuiControl, Show, No_Sound
		GuiControl, Show, Gentle_Sound
		GuiControl, Show, Ocean_Waves
		GuiControl, Show, Persistent_Sound
		GuiControl, Show, Heart_Beat
		GuiControl, Show, Music_Alert
		GuiControl, Show, Set_Idle_Time
		GuiControl, Show, BtnScanRefresh
		GuiControl, Show, BtnCheckObject
		GuiControl, Show, BtnVideo
		GuiControl, Show, BtnGitHub
		GuiControl, Show, BtnAbout
		GuiControl, Show, Tick
		GuiControl, Show, Seconds
		Gui, Show, w245 h320, Idle Alert
		GoSub, Destroy_Alert
	; Saving Input Data into INI File
		GuiControlGet, Tick
		StringReplace, Tick, Tick, `n, ``n, All
...
;----------------------------------------------- Submit Checkbox - Flash Alert

Submit_Checkbox_Flash:
	SoundPlay, %Button_Click%
	Gui, Submit, NoHide
	If(Flash_Alert = 1)
	{
		GuiControlGet, Flash_Alert
		StringReplace, Flash_Alert, Flash_Alert, `n, ``n, All
		IniWrite, %Flash_Alert%, Settings.ini, Flash Alert, Flash Alert
		GuiControl, , Flash_Alert, 1
		GuiControl, Enable, Monitor_1
		GuiControl, Enable, Monitor_2
		GuiControl, Enable, Monitor_3
...
;----------------------------------------------- Submit Radio - Flash Alert Monitor

Submit_Radio_Monitor:
	SoundPlay, %Button_Click%
	Gui, Submit, NoHide
	If(Monitor_1 = 1)
	{
	; Enable Primary Monitor
		GuiControlGet, Monitor_1
		StringReplace, Monitor_1, Monitor_1, `n, ``n, All
		IniWrite, %Monitor_1%, %A_ScriptDir%\Settings.ini, Flash Alert, Primary Monitor
		GuiControl, , Monitor_1, 1
	; Disable Secondary Monitor
		Monitor_2 := 0
		StringReplace, Monitor_2, Monitor_2, `n, ``n, All
		IniWrite, %Monitor_2%, %A_ScriptDir%\Settings.ini, Flash Alert, Secondary Monitor
	; Disable All Monitors
		Monitor_3 := 0
		StringReplace, Monitor_3, Monitor_3, `n, ``n, All
		IniWrite, %Monitor_3%, %A_ScriptDir%\Settings.ini, Flash Alert, All Monitors
	}
	If(Monitor_2 = 1)
	{
	; Enable Secondary Monitor
		GuiControlGet, Monitor_2
		StringReplace, Monitor_2, Monitor_2, `n, ``n, All
		IniWrite, %Monitor_2%, %A_ScriptDir%\Settings.ini, Flash Alert, Secondary Monitor
		GuiControl, , Monitor_2, 1
...
Submit_Radio_Sound:
	...
	{
	; Enable No Sound
		SoundPlay, %Crickets_Chirping%
		GuiControlGet, No_Sound
		StringReplace, No_Sound, No_Sound, `n, ``n, All
		IniWrite, %No_Sound%, %A_ScriptDir%\Settings.ini, Sound Alert, No Sound
		GuiControl, , No_Sound, 1
	; Disable Gentle Sound
		Gentle_Sound := 0
		StringReplace, Gentle_Sound, Gentle_Sound, `n, ``n, All
		IniWrite, %Gentle_Sound%, %A_ScriptDir%\Settings.ini, Sound Alert, Gentle Sound
	; Disable Ocean Waves
		Ocean_Waves := 0
		StringReplace, Ocean_Waves, Ocean_Waves, `n, ``n, All
		IniWrite, %Ocean_Waves%, %A_ScriptDir%\Settings.ini, Sound Alert, Ocean Waves
	; Disable Persistent Sound
		Persistent_Sound := 0
		StringReplace, Persistent_Sound, Persistent_Sound, `n, ``n, All
		IniWrite, %Persistent_Sound%, %A_ScriptDir%\Settings.ini, Sound Alert, Persistent Sound
	; Disable Heart Beat
		Heart_Beat := 0
		StringReplace, Heart_Beat, Heart_Beat, `n, ``n, All
		IniWrite, %Heart_Beat%, %A_ScriptDir%\Settings.ini, Sound Alert, Heart Beat
	; Disable Music Alert
		Music_Alert := 0
		StringReplace, Music_Alert, Music_Alert, `n, ``n, All
		IniWrite, %Music_Alert%, %A_ScriptDir%\Settings.ini, Sound Alert, Music Alert
	}
	If(Gentle_Sound = 1)
	{
	; Enable Gentle Sound
		SoundPlay, %Wind_Blowing%
		GuiControlGet, Gentle_Sound
		StringReplace, Gentle_Sound, Gentle_Sound, `n, ``n, All
		IniWrite, %Gentle_Sound%, %A_ScriptDir%\Settings.ini, Sound Alert, Gentle Sound
		GuiControl, , Gentle_Sound, 1
	; Disable No Sound
	...
	{
	; Enable Ocean Waves
		SoundPlay, %Ocean_Waves_Sound%
		GuiControlGet, Ocean_Waves
		StringReplace, Ocean_Waves, Ocean_Waves, `n, ``n, All
		IniWrite, %Ocean_Waves%, %A_ScriptDir%\Settings.ini, Sound Alert, Ocean Waves
		GuiControl, , Ocean_Waves, 1
	; Disable No Sound
	...
	If(Persistent_Sound = 1)
	{
	; Enable Persistent Sound
		SoundPlay, %Beep_Sound%
		GuiControlGet, Persistent_Sound
		StringReplace, Persistent_Sound, Persistent_Sound, `n, ``n, All
		IniWrite, %Persistent_Sound%, %A_ScriptDir%\Settings.ini, Sound Alert, Persistent Sound
		GuiControl, , Persistent_Sound, 1
	; Disable No Sound
	...
	{
	; Enable Heart Beat
		SoundPlay, %Heart_Beat_Sound%
		GuiControlGet, Heart_Beat
		StringReplace, Heart_Beat, Heart_Beat, `n, ``n, All
		IniWrite, %Heart_Beat%, %A_ScriptDir%\Settings.ini, Sound Alert, Heart Beat
		GuiControl, , Heart_Beat, 1
	; Disable No Sound
	...
	{
	; Enable Music Alert
		SoundPlay, %Music_Sound%
		GuiControlGet, Music_Alert
		StringReplace, Music_Alert, Music_Alert, `n, ``n, All
		IniWrite, %Music_Alert%, %A_ScriptDir%\Settings.ini, Sound Alert, Music Alert
		GuiControl, , Music_Alert, 1
	; Disable No Sound
...
Idle_State:
	...
	Gui, Font, s16 cRed Norm Bold, Verdana
	GuiControl, Font, Work_Status
	If (Ok:=FindText(1090-150000, 452-150000, 1090+150000, 452+150000, 0, 0, 1Text)) OR (Ok:=FindText(1090-150000, 452-150000, 1090+150000, 452+150000, 0, 0, 2Text))
...
Working_State:
	...
	Else
		i := 0
	For i,v in Ok
	GuiControl, , Work_Status, Working
	Gui, Font, s16 cGreen Norm Bold, Verdana
	GuiControl, Font, Work_Status
	If (i<1)
	{
		SetTimer, Working_State, Off
		GoSub, Start_Idle_Countdown
	}
	Return

;-------------------- Start Idle Countdown Idle

Start_Idle_Countdown:
	Idle_Countdown := 1
	SetTimer, Idle_Countdown, 1000
	GoSub, Submit_Time
	Return
...
Idle_Countdown:
	{
		Tick--
		GuiControl, , Work_Status, Idle - %Tick%
		Gui, Font, s16 cYellow Norm Bold, Verdana
		GuiControl, Font, Work_Status
	}
	If Tick = 0
	{
...
Play_Alert:
	...
	{
		If(Monitor_1 = 1)
			Run, %A_ScriptDir%\Visual\Flash Alert - Primary Monitor.exe
		If(Monitor_2 = 1)
			Run, %A_ScriptDir%\Visual\Flash Alert - Secondary Monitor.exe
		If(Monitor_3 = 1)
			Run, %A_ScriptDir%\Visual\Flash Alert - All Monitors.exe
	}
	If(No_Sound = 1)
		Return
	If(Gentle_Sound = 1)
		Run, %A_ScriptDir%\Sound\Alert\Gentle Sound\Gentle Sound.exe
	If(Ocean_Waves = 1)
		Run, %A_ScriptDir%\Sound\Alert\Ocean Waves\Ocean Waves.exe
	If(Persistent_Sound = 1)
		Run, %A_ScriptDir%\Sound\Alert\Persistent Sound\Persistent Sound.exe
	If(Heart_Beat = 1)
		Run, %A_ScriptDir%\Sound\Alert\Heart Beat\Heart Beat.exe
	If(Music_Alert = 1)
		Run, %A_ScriptDir%\Sound\Alert\Music\Music Alert.exe
	Return

;----------------------------------------------- Destroy Alert

Destroy_Alert:
	If WinExist("Flash Alert - Primary Monitor.exe")
	{
		Destroy_Flash_Alert_Primary = %A_ScriptDir%\Visual\Flash Alert - Primary Monitor.exe
		WinClose, %Destroy_Flash_Alert_Primary% ahk_class AutoHotkey
	}
	If WinExist("Flash Alert - Secondary Monitor.exe")
	{
		Destroy_Flash_Alert_Secondary = %A_ScriptDir%\Visual\Flash Alert - Secondary Monitor.exe
		WinClose, %Destroy_Flash_Alert_Secondary% ahk_class AutoHotkey
	}
	If WinExist("Flash Alert - All Monitors.exe")
	{
		Destroy_Flash_Alert_All = %A_ScriptDir%\Visual\Flash Alert - All Monitors.exe
		WinClose, %Destroy_Flash_Alert_All% ahk_class AutoHotkey
	}
	If WinExist("Gentle Sound.exe")
	{
		Destroy_Gentle_Sound = %A_ScriptDir%\Sound\Alert\Gentle Sound\Gentle Sound.exe
		WinClose, %Destroy_Gentle_Sound% ahk_class AutoHotkey
	}
	If WinExist("Ocean Waves.exe")
	{
		Destroy_Ocean_Waves = %A_ScriptDir%\Sound\Alert\Ocean Waves\Ocean Waves.exe
		WinClose, %Destroy_Ocean_Waves% ahk_class AutoHotkey
	}
	If WinExist("Persistent Sound.exe")
	{
		Destroy_Persistent_Sound = %A_ScriptDir%\Sound\Alert\Persistent Sound\Persistent Sound.exe
		WinClose, %Destroy_Persistent_Sound% ahk_class AutoHotkey
	}
	If WinExist("Heart Beat.exe")
	{
		Destroy_Heart_Beat = %A_ScriptDir%\Sound\Alert\Heart Beat\Heart Beat.exe
		WinClose, %Destroy_Heart_Beat% ahk_class AutoHotkey
	}
	If WinExist("Music Alert.exe")
	{
		Destroy_Music_Alert = %A_ScriptDir%\Sound\Alert\Music\Music Alert.exe
		WinClose, %Destroy_Music_Alert% ahk_class AutoHotkey
	}
	Return
