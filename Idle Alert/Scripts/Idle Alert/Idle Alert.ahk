...
; Delay Idle Time - Seconds
	IniRead, Tick, %A_ScriptDir%\Settings.ini, Delay Idle Time, Seconds, %A_Space%
...
; Edit Box - Set Delay Idle Time
	Gui, Font, Norm Bold, Verdana
	Gui, Font, s10, Verdana
	Gui, Add, Text, c00468B x5 y66 vSet_Idle_Time, Idle Time:
	Gui, Font, s10, Verdana
	Gui, Add, Edit, Limit2 Number x+4 w36 h20 vTick gSubmit_Initial_Time, %Tick%
	Gui, Font, s8, Verdana
	Gui, Add, Text, cBlack x+4 vSeconds, seconds
; Checkbox - Flash Alert
	Gui Font, s8, Verdana
	Gui, Add, CheckBox, c8B0000 x5 y92 vFlash_Alert gSubmit_Checkbox, - Flash Alert
; Gui Radio Button
	Gui Font, s8, Verdana
	Gui, Add, Radio, c444444 x5 y115 vNo_Sound gSubmit_Radio, - No Sound
	Gui, Add, Radio, c228B22 x5 y+5 vGentle_Sound gSubmit_Radio, - Gentle Sound
	Gui, Add, Radio, c2B65EC x5 y+5 vOcean_Waves gSubmit_Radio, - Ocean Waves
	Gui, Add, Radio, c5A5A58 x5 y+5 vPersistent_Sound gSubmit_Radio, - Persistent Sound
	Gui, Add, Radio, cC11C84 x5 y+5 vHeart_Beat gSubmit_Radio, - Heart Beat
	Gui, Add, Radio, cAA6C39 x5 y+5 vMusic_Alert gSubmit_Radio, - Music Alert
; Gui Close Button
	Gui, Font, s10, Verdana
	Gui, Add, Picture, x219 y2 w24 h24 vClose_Window gGuiClose, %A_ScriptDir%\Images\Close Window.png
; INI Settings - Checkbox
	IniRead, Flash_Alert, Settings.ini, Flash, Flash Alert, %A_Space%
	If (Flash_Alert = 0)
		GuiControl, , Flash_Alert, 0
	If (Flash_Alert = 1)
		GuiControl, , Flash_Alert, 1
; INI Settings - Radio Button
	IniRead, No_Sound, %A_ScriptDir%\Settings.ini, Sound, No Sound, %A_Space%
	If (No_Sound = 0)
		GuiControl, , No_Sound, 0
	If (No_Sound = 1)
		GuiControl, , No_Sound, 1
	IniRead, Gentle_Sound, %A_ScriptDir%\Settings.ini, Sound, Gentle Sound, %A_Space%
	If (Gentle_Sound = 0)
		GuiControl, , Gentle_Sound, 0
	If (Gentle_Sound = 1)
		GuiControl, , Gentle_Sound, 1
	IniRead, Ocean_Waves, %A_ScriptDir%\Settings.ini, Sound, Ocean Waves, %A_Space%
	If (Ocean_Waves = 0)
		GuiControl, , Ocean_Waves, 0
	If (Ocean_Waves = 1)
		GuiControl, , Ocean_Waves, 1
	IniRead, Persistent_Sound, %A_ScriptDir%\Settings.ini, Sound, Persistent Sound, %A_Space%
	If (Persistent_Sound = 0)
		GuiControl, , Persistent_Sound, 0
	If (Persistent_Sound = 1)
		GuiControl, , Persistent_Sound, 1
	IniRead, Heart_Beat, %A_ScriptDir%\Settings.ini, Sound, Heart Beat, %A_Space%
	If (Heart_Beat = 0)
		GuiControl, , Heart_Beat, 0
	If (Heart_Beat = 1)
		GuiControl, , Heart_Beat, 1
	IniRead, Music_Alert, %A_ScriptDir%\Settings.ini, Sound, Music Alert, %A_Space%
	If (Music_Alert = 0)
		GuiControl, , Music_Alert, 0
	If (Music_Alert = 1)
		GuiControl, , Music_Alert, 1
; Gui Finalize
	Draw_Outline(Gui_Name := 1, x := 0, y := 0, w := 245, h := 225, Color1 := "AA336A", Color2 := "AA336A", Thickness := 2)
	Gui, Show, x%xPos% y%yPos% w245 h225, Idle Alert
...
ScanRefresh:
	...
	; Saving Input Data into INI File
		...
		IniWrite, %Tick%, %A_ScriptDir%\Settings.ini, Delay Idle Time, Seconds
		Gui, Destroy
		Sleep, 75
		Reload
	}
...
Submit_Checkbox:
	...
	If(Flash_Alert = 1)
	{
		...
		IniWrite, %Flash_Alert%, Settings.ini, Flash, Flash Alert
		GuiControl, , Flash_Alert, 1
	}
	Else
	{
...
Submit_Radio:
	...
	; Enable No Sound
		...
		IniWrite, %No_Sound%, %A_ScriptDir%\Settings.ini, Sound, No Sound
		GuiControl, , No_Sound, 1
	; Disable Gentle Sound
		Gentle_Sound := 0
		StringReplace, Gentle_Sound, Gentle_Sound, `n, ``n, All
		IniWrite, %Gentle_Sound%, %A_ScriptDir%\Settings.ini, Sound, Gentle Sound
	; Disable Ocean Waves
		Ocean_Waves := 0
		StringReplace, Ocean_Waves, Ocean_Waves, `n, ``n, All
		IniWrite, %Ocean_Waves%, %A_ScriptDir%\Settings.ini, Sound, Ocean Waves
	; Disable Persistent Sound
		Persistent_Sound := 0
		StringReplace, Persistent_Sound, Persistent_Sound, `n, ``n, All
		IniWrite, %Persistent_Sound%, %A_ScriptDir%\Settings.ini, Sound, Persistent Sound
	; Disable Heart Beat
		Heart_Beat := 0
		StringReplace, Heart_Beat, Heart_Beat, `n, ``n, All
		IniWrite, %Heart_Beat%, %A_ScriptDir%\Settings.ini, Sound, Heart Beat
	; Disable Music Alert
		Music_Alert := 0
		StringReplace, Music_Alert, Music_Alert, `n, ``n, All
		IniWrite, %Music_Alert%, %A_ScriptDir%\Settings.ini, Sound, Music Alert
	}
	If(Gentle_Sound = 1)
	{
	; Enable Gentle Sound
...
Play_Alert:
	If(Flash_Alert = 1)
		Run, %A_ScriptDir%\Visual\Flash Alert.exe
	If(No_Sound = 1)
		Return
	If(Gentle_Sound = 1)
...
Destroy_Alert:
	If WinExist("Flash Alert.exe")
	{
		Destroy_Flash_Alert = %A_ScriptDir%\Visual\Flash Alert.exe
		WinClose, %Destroy_Flash_Alert% ahk_class AutoHotkey
	}
